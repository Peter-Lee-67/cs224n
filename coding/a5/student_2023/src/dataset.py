...
class NameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # inp[-1] -> self.MASK_CHAR (Lee)
        # Using the PAD_CHAR characters in y before the ⁇[place] keeps the trainer from \
        # optimizing the model to predict the question, "Where was...".
        
        x = x[:-1]
        # len(x) = len(y) = (self.block_size - 1)
...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_doc = doc[:truncated_length]

        # 2. break the (truncated) document into [prefix] [masked_content] [suffix]
        # The length of [masked_content] should be random, and 1/4 the length of the
        # truncated document on average
        masked_length = random.randint(int(len(truncated_doc)/8), int(len(truncated_doc)*3/8))
        start_idx = random.randint(0, len(truncated_doc) - masked_length) # suppose [prefix] and [suffix] could be null.(Lee)

        # 3. Rearrange these substrings into [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] [pads]
        prefix = truncated_doc[:start_idx]
        masked_content = truncated_doc[start_idx:start_idx+masked_length]
        suffix = truncated_doc[start_idx+masked_length:]
        masked_doc = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.MASK_CHAR # suppose [masked_content] is surrounded by MASK_CHAR(Lee)
        masked_doc = masked_doc + (self.block_size - len(masked_doc)) * self.PAD_CHAR

        # 4. take the input string to be masked_string[:-1], and the output
        # string to be masked_string[1:]
        x, y = masked_doc[:-1], masked_doc[1:]
