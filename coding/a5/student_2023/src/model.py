...
class DownProjectBlock(nn.Module):
    ...
    def forward(self, x_input):
        ...
        x_q, x_kv = self.ln1(self.C), x_input
        x = x_q + self.attn(x_kv, x_q)  # (batch_size, bottleneck_dim, n_embd)
        x = x + self.mlp(self.ln2(x))
        return x
        # pass
        ### END YOUR CODE
...
class UpProjectBlock(nn.Module):
    ...
    def forward(self, y, x_input):
        ...
        ### Hint: Copy over the code from Block and make necessary modifications.
        ### Should be around 3-5 lines.
        x_q, x_kv = x_input, self.ln1(y)
        x = x_q + self.attn(x_kv, x_q)  # (batch_size, sentence_length, n_embd)
        x = x + self.mlp(self.ln2(x))
        return x
        # pass
        ### END YOUR CODE
